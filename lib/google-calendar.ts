...
function privateAvailabilityDescription(session: PrivateSessionAvailabilityRow) {
  return [
    ...
    `Payment status: ${session.payment_status || (session.status === "booked" ? "Paid" : "Not booked")}`,
    `${session.payment_status === "zelle_pending" ? "Amount due" : "Amount paid"}: ${formatCurrencyFromCents(session.amount_paid || 0)}`,
    `Waiver status: ${session.waiver_signed ? "Signed" : "Missing"}`,
    `Typed waiver signature: ${session.typed_signature || "Not recorded"}`,
    `Waiver signed date/time: ${session.signed_at || "Not recorded"}`,
